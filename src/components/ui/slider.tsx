...
function Slider({ className, value, min = 0, explicitMin = min, max = 100, step = 1, variant = 'default', orientation = 'horizontal', notchDivisor = 1, onValueChange, snapDistance = 0, ...props }: SliderProps) {
  ...
  return (
    <SliderPrimitive.Root
      ...
      className={cn('relative flex touch-none select-none items-center data-disabled:opacity-45 data-[orientation=horizontal]:w-full data-[orientation=vertical]:h-full data-[orientation=vertical]:flex-col', variant === 'transport' && 'h-12 cursor-pointer', className)}
      {...props}
    >
      <SliderPrimitive.Track
        ...
        className={cn(
          ...
          variant === 'transport' && 'rounded-none border-x border-border/70 bg-muted/45 data-[orientation=horizontal]:h-full',
        )}
      >
        {variant === 'notched' && (
        ...
        <SliderPrimitive.Range data-slot="slider-range" className={cn('bg-primary absolute data-[orientation=horizontal]:h-full data-[orientation=vertical]:w-full rounded-full', variant === 'transport' && 'bg-transparent')} />
      </SliderPrimitive.Track>
      {mappedValue.map((_, index) => (
        <SliderPrimitive.Thumb
          data-slot="slider-thumb"
          key={index}
          className={cn('block size-4 rounded-full border border-primary bg-background shadow outline-none focus-visible:ring-3 focus-visible:ring-ring/40', variant === 'transport' && 'relative z-10 h-full w-5 rounded-none border-0 bg-transparent shadow-none ring-0 focus-visible:ring-0')}
        />
      ))}
    </SliderPrimitive.Root>
  );
